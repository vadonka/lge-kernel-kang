/*
 * arch/arm/mach-tegra/odm_kit/query/whistler/nvodm_query_discovery.c
 *
...
#include "subboards/nvodm_query_discovery_e1219_addresses.h"
...
#include "subboards/nvodm_query_discovery_e1219_peripherals.h"
...
NvIsFilteredPeripheral(const NvOdmPeripheralConnectivity* pConnectivity)
{
    ...
    if (pConnectivity->Guid == NV_ODM_GUID('b','l','u','t','o','o','t','h'))
    {
         ...
         * In case of whistler only ossibility is having E1219 board.
         */
         status = NvOdmPeripheralGetBoardInfo((BOARD_ID_E1219), &BoardInfo);
         if(NV_TRUE == status)
         {
