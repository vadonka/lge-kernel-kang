...
// 20100906 joseph.jung@lge.com Touch F/W version [END]
	...
	hTouch->FirmwareRevId = 0xFF;

	NvOdmOsWaitUS(200*1000); // wait for 100ms

	for(i = 0; i < 5; i++)
	{
		Synaptics_ReadRegisterSafe(hTouch, 0xB0, (NvU8*)(&old_ver), sizeof(old_ver));
		printk("[TOUCH] Touch controller Firmware Version drop Check = %x\n", old_ver);
		NvOdmOsWaitUS(1000); // wait for 1ms
	}
	...
	Synaptics_ReadRegisterSafe(hTouch, 0xB0, (NvU8*)(&old_ver), sizeof(old_ver));
	printk("[TOUCH] Touch controller Firmware Version 1st Check = %x\n", old_ver);
	NvOdmOsWaitUS(1000); // wait for 1ms

	Synaptics_ReadRegisterSafe(hTouch, 0xB0, (NvU8*)(&prev_ver), sizeof(prev_ver));
	printk("[TOUCH] Touch controller Firmware Version 2nd Check = %x\n", prev_ver);
	NvOdmOsWaitUS(1000); // wait for 1ms

	for(i = 0; i < 3; i++)
	{
		Synaptics_ReadRegisterSafe(hTouch, 0xB0, (NvU8*)(&curr_ver), sizeof(curr_ver));
		printk("[TOUCH] Touch controller Firmware Version new Check = %x\n", curr_ver);

		if((old_ver == prev_ver) && (prev_ver == curr_ver) && (curr_ver != 0xFF))
			break;
