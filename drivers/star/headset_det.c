...
static struct wake_lock headset_wake_lock;  //20111017  Problem that no wake up when disconn headset in calling
...
static void headset_int_handler(void *dev_id)
{
	...
	{
	    schedule_delayed_work(&headset_sw_data->delayed_work,	msecs_to_jiffies(type_detection_time));	
	    wake_lock_timeout(&headset_wake_lock, msecs_to_jiffies(type_detection_time + 50));	 //20111017  Problem that no wake up when disconn headset in calling
    }
...
/*====================== nVidia GPIO Control(E) =======================*/
	...
	wake_lock_init(&headset_wake_lock, WAKE_LOCK_SUSPEND, "headset_wlock"); //20111017  Problem that no wake up when disconn headset in calling
...
static int headsetdet_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&headset_wake_lock);		//20111017  Problem that no wake up when disconn headset in calling
/*====================== nVidia GPIO Control(S) =======================*/
    NvOdmGpioInterruptUnregister(s_hHeadsetHandle.hGpio, s_hHeadsetHandle.h_Headset_Detection, s_hHeadsetHandle.hheadsetInterrupt);
...
//20101117, , gpio wakeup from LP1 [END]
    ...
    if(core_lock_on){
	  ...
	  cancel_delayed_work_sync(&headset_sw_data->delayed_work); //20111017  Problem that no wake up when disconn headset in calling
