...
//struct ami304mid_data *compass_dev;
struct compass_data {
	NvU32 x;
	NvU32 y;
	NvU32 z;
...
NvBool NvGyroAccelI2CSetRegsPassThrough(NvU8 offset, NvU8* value, NvU32 len)
{
	
	NvCompassI2CSetRegs(compass_dev->hOdmComp, offset, value, len);
}

NvBool NvGyroAccelI2CGetRegsPassThrough(NvU8 offset, NvU8* value, NvU32 len)
{
	
	NvCompassI2CGetRegs(compass_dev->hOdmComp, offset, value, len);
}
...
static NvS32 __init ami304_probe(struct platform_device *pdev)
{
	...
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_chipinfo);
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_sensordata);
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_posturedata);
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_calidata);
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_midcontrol);
	err = device_create_file(&compass_dev->input_dev->dev, &dev_attr_mode);
