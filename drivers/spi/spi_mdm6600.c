...
//20100811, syblue.lee@lge.com, move global variables [END]
#ifdef WAKE_LOCK_RESUME
...
//#endif
	...
	if (index == 0) {
		...
		port_temp = 'h'-'a';
		pin = 2;
		s_modemCheck.pinHandle = NvOdmGpioAcquirePinHandle(s_modemCheck.gpioHandle, port_temp, pin);
		...
		NvOdmGpioConfig(s_modemCheck.gpioHandle, s_modemCheck.pinHandle, NvOdmGpioPinMode_Output);
#endif
...
ifx_spi_late_resume(struct early_suspend *h)
{
	...
	//			wake_lock_timeout(&gspi_data[0]->wake_lock, 50);	//20101203-1, syblue.lee@lge.com, change 3 to 1 for power consumption
	//#endif		
			printk("[IFX_SRDY] wakeup pad : 0x%lx", reg);
...
//		writel(WAKEUP_IFX_SRDY_MASK, pmc_base + PMC_WAKE_STATUS); // clearing  wakeup bit 
		...
		if(&gspi_data[0]->wake_lock)
			wake_lock_timeout(&gspi_data[0]->wake_lock, 5);	//20101203-1, syblue.lee@lge.com, change 3 to 1 for power consumption
#endif		
...
static void ifx_spi_handle_mrdy_irq(void *handle)
{
	...
	if (ifx_shutdown) return ;
	if(spi_data && spi_data->ifx_tty)	//20101104-1, syblue.lee@lge.com, prevent MRDY until spi openning
	{	//20101104-1, syblue.lee@lge.com, prevent MRDY until spi openning
// EBS LGE_UPDATE_S eungbo.shim@lge.com - 20110610 
#if 1//def CONFIG_HAS_WAKELOCK
    	wake_lock_timeout(&gspi_data[0]->wake_lock, 5);	
#endif
		queue_work(spi_data->ifx_wq, &spi_data->ifx_work);
// EBS LGE_UPDATE_E eungbo.shim@lge.com - 20110610  
	}
	else
	{
...
ifx_spi_handle_work(struct work_struct *work)
{
	...
	int id = 0;	//20101104-2, syblue.lee@lge.com, ignore this value
	unsigned long diff;
	unsigned long reg;
	int pm_off_count;
	if (ifx_shutdown) return;
